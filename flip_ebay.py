...
class flipkart:
    def __init__(self,product,pageNum,url='https://www.flipkart.com/search?q='):
        self.product=product
        self.url=url
        self.pageNum=pageNum
    
    def connect(self):
        final_url=self.url+self.product+'&page='+str(self.pageNum)
        try:
...
class ebay:
    def __init__(self,product,pageNum,url='https://www.ebay.com/sch/i.html?_nkw='):
        self.product=product
        self.url=url
        self.pageNum=pageNum

    def connect(self):
        final_url=self.url+self.product+'&_pgn='+str(self.pageNum)
        try:
            response=urlopen(final_url).read().decode('utf-8')
            return response
        except:
...
if __name__ == "__main__":
	...
	if site not in [1,2]:
		...
		if site==1:
			flip=flipkart(product,page_num)
			flip.get_urls()
		elif site==2:
			eb=ebay(product,page_num)
			eb.get_urls()